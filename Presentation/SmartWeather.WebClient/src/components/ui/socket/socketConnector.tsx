...
import {
  getAllStation,
  getStationsMeasurePoints,
} from "@/services/station/station_service";
...
function socketConnector() {
  const [lastUpdatedMeasurePoints, setlastUpdatedMeasurePoints] =
    useState<MeasurePointDataDto>({ id: 0, value: 0 });
  const [stationMeasurePointsMap, setStationMeasurePointsMap] = useState<
    StationMeasurePointsResponse[]
  >([]);
  ...
  useEffect(() => {
    const initStations = async () => {
      ...
      if (
        stationsResponse.status == Status.OK &&
        stationsResponse.data != null
      ) {
  ...
  useEffect(() => {
    const initSocketConnection = async () => {
      ...
      try {
        signalRService.invoke<MeasurePointDataDto>(
          "InitiateMonitoring",
          monitoringReq
        );
        signalRService.on(
          "receivedMeasurePointData",
          (newMeasurePoint: MeasurePointDataDto) => {
  ...
  return (
    <div>
      <MeanDataMetrics
        stationMeasurePointsMap={stationMeasurePointsMap}
        lastUpdatedMeasurePoints={lastUpdatedMeasurePoints}
        stations={stations}
      />
    </div>
  );
